...
#Modify the region of interest if needed
...
for sub in range (30):
    ...
    for film in movies_list:
        film_='Computation/Within/'+ region_interest +'/' + film +'_' + region_interest+ '_dyn_connectivity_width_30.pkl'
...
with open('Computation/Pairwise/'+region_interest+'/' + region_interest +'_all_movies_within_subjects_analysis.pkl', 'wb') as f:
    pd.to_pickle(list_dataframe, f)


def blockwide_corr(sub_a_index, sub_b_index):
    #Load data
    with open('Computation/Pairwise/'+region_interest+'/' + region_interest +'_all_movies_within_subjects_analysis.pkl', "rb") as file:
        sub_data = pickle.load(file)
...
pkl_file = open('Computation/Pairwise/'+region_interest+'/' + region_interest +'_all_movies_within_subjects_analysis.pkl', "rb")
...
TR_all_movies = 7371
r_ref_l = l_r_region_interest[0]+'_x_'
r_ref_r = l_r_region_interest[1]+'_x_'
r_interest_TR = [r_ref_l+'7Networks_LH_SalVentAttn_Med_1', r_ref_l+'7Networks_RH_SalVentAttn_Med_1',
                ...
                r_ref_l+'Left Amygdala', r_ref_l+'Right Amygdala',
                ...
                r_ref_r+'Left Amygdala', r_ref_r+'Right Amygdala']
#remove duplicated region
duplicate_l =r_ref_l+l_r_region_interest[0]
duplicate_r =r_ref_r+l_r_region_interest[1]
r_interest_TR.remove(duplicate_l)
r_interest_TR.remove(duplicate_r)
...
#Run the part below if Behavioural_PSY_scored.xlsx was not already ordered
...
#Run the line below if Behavioural_PSY_scored.xlsx was already ordered
sub_ordered_behavior = pd.read_excel('ordered_by_sub_behavior_score.xlsx')

#select behavior of interest
start_column_name = "cov_total"  # the name of the column we want to start from
...
pkl_file = open('Computation/Pairwise/'+region_interest+'/'+region_interest+'_Pairwise_Data_all_movie.pkl', "wb")
