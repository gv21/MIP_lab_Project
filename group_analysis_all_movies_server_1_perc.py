...
# Function to generate null distribution = FROM THE PAPER
"""def null_dist_paper(paired_data, perms=500):
    #print('rng')
...
num_perms = 1000 ###nbr of permutation to build the distribution 
...
with Pool (processes=nbr) as pool: #processes = number of thread (coeur) I use on my computer
    result = pool.starmap(computation_movie_wise, connection_x_behavior[:17693]) #take 1% of the data 
...
pkl_file = open("Computation/Group/all_amygd_Movie_wise_group_analysis_1_per.pkl", "wb")
pickle.dump(statistical_tests, pkl_file)
pkl_file.close()
statistical_tests.to_csv('Computation/Group/all_amygd_Movie_wise_group_analysis_1_per.csv', index=False)
